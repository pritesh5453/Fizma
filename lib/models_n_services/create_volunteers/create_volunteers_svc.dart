...
class VolunteerService {
  ...
  // ---------- CREATE VOLUNTEER (NEW) ----------
  Future<CreateVolunteerResponse> createVolunteer(
    CreateVolunteerRequest request,
  ) async {
    try {
      final response = await _dio.post(
        ApiEndpoints.createVolunteer(), // make sure this endpoint exists
        data: request.toJson(),
      );

      print("=========== CREATE VOLUNTEER SUCCESS ===========");
      print("Status : ${response.statusCode}");
      print("Data   : ${response.data}");
      print("=================================================");

      if (response.statusCode == 200 || response.statusCode == 201) {
        return CreateVolunteerResponse.fromJson(response.data);
      } else {
        throw Exception('Server error: ${response.statusCode}');
      }
    } on DioException catch (e) {
      print("=========== CREATE VOLUNTEER ERROR ===========");
      print("Status Code : ${e.response?.statusCode}");
      print("Response    : ${e.response?.data}");
      print("Message     : ${e.message}");
      print("==============================================");
